...
def get_by_class_saliency_abs(image_path,
                              label,
                              operations,
                              model_name='densnet',
                              layers=[12],
    ...
    if mode == TYPES[0]:
        images, integrated_heatmaps = heatmap_of_layer_abs_normal(device, image_path, label,
                                                                  layers,
                                                                  model_name)
    elif mode == TYPES[1]:
        images, integrated_heatmaps = heatmap_of_layer_abs_uniform(device, image_path, label,
                                                                   layers,
                                                                   model_name)
    elif mode == TYPES[2]:
        images, integrated_heatmaps = heatmap_of_layer_abs_blur(device, image_path, label,
                                                                layers,
                                                                model_name)
    elif mode == TYPES[3]:
        images, integrated_heatmaps = heatmap_of_layer_abs_constant(device, image_path, label,
                                                                    layers,
                                                                    model_name)
    else:
...
def get_by_class_saliency_abs_traindata(image_path,
                                        label,
                                        operations,
                                        model_name='densnet',
                                        layers=[12],
    ...
    images, integrated_heatmaps = heatmap_of_layer_abs_traindata(device, image_path, label,
                                                                 layers,
...
def heatmap_of_layer_abs_normal(device, image_path, label, layers, model_name):
    model = GradModel(model_name, feature_layer=layers[0])
    model.to(device)
    model.eval()
    model.zero_grad()
    ...
    full_tensor = get_noise_values_abs(model, activations, label)
...
def heatmap_of_layer_abs_uniform(device, image_path, label, layers, model_name):
    model = GradModel(model_name, feature_layer=layers[0])
    model.to(device)
    model.eval()
    model.zero_grad()
    ...
    full_tensor = get_noise_values_abs(model, activations, label, mode=UNIFORM)
...
def heatmap_of_layer_abs_blur(device, image_path, label, layers, model_name):
    model = GradModel(model_name, feature_layer=layers[0])
    model.to(device)
    model.eval()
    model.zero_grad()
...
def heatmap_of_layer_abs_traindata(device, image_path, label, layers, model_name, images_df):
    model = GradModel(model_name, feature_layer=layers[0])
    model.to(device)
    model.eval()
    model.zero_grad()
...
def heatmap_of_layer_abs_constant(device, image_path, label, layers, model_name):
    model = GradModel(model_name, feature_layer=layers[0])
    model.to(device)
    model.eval()
    model.zero_grad()
...
def get_noise_values_abs(model, activation, label, device='cuda', mode=NORMAL):
    ...
    interpolated_activations = abs_baseline(model, label, device, activation, perturb_activation)

    return interpolated_activations


def abs_baseline(model, label, device, activations, pert_activation_last):
...
if __name__ == '__main__':
    ...
    operations = [ABS + '_' + TYPES[0], ABS + '_' + TYPES[1], ABS + '_' + TYPES[2], ABS + '_' + TYPES[3],
                  ABS + '_' + TYPES[4]]
    operations = [ABS + '_' + TYPES[0]]
    OPERATION_NUM = 3
    ...
    for index, row in tqdm(df.iterrows()):
        ...
        for sample_repeater in range(TIMES_TO_REPEAT):
            ...
            img_dict = []
            for operation in operations:
                if operation.__contains__(ABS + '_' + TYPES[0]):
                    t, blended_im, heatmap_cv, blended_img_mask, image, score, heatmap = \
                        get_by_class_saliency_abs(image_path=image_path,
                                                  label=label,
                ...
                elif operation.__contains__(ABS + '_' + TYPES[1]):
                    t, blended_im, heatmap_cv, blended_img_mask, image, score, heatmap = \
                        get_by_class_saliency_abs(image_path=image_path,
                                                  label=label,
                                                  operations=[operation],
                ...
                elif operation.__contains__(ABS + '_' + TYPES[2]):
                    t, blended_im, heatmap_cv, blended_img_mask, image, score, heatmap = \
                        get_by_class_saliency_abs(image_path=image_path,
                                                  label=label,
                                                  operations=[operation],
                ...
                elif operation.__contains__(ABS + '_' + TYPES[3]):
                    t, blended_im, heatmap_cv, blended_img_mask, image, score, heatmap = \
                        get_by_class_saliency_abs(image_path=image_path,
                                                  label=label,
                                                  operations=[operation],
                ...
                elif operation.__contains__(ABS + '_' + TYPES[4]):
                    t, blended_im, heatmap_cv, blended_img_mask, image, score, heatmap = \
                        get_by_class_saliency_abs_traindata(image_path=image_path,
                                                            label=label,
                                                            operations=[operation],
                                                            model_name=model_name,
                                                            layers=[FEATURE_LAYER_NUMBER],
